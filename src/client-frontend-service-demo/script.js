...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('.search-btn').addEventListener('click', function() {
        const searchQuery = document.getElementById('restaurant-search').value.trim();
        if (searchQuery) {
...
function displayRestaurants(restaurants) {
    ...
    restaurants.forEach(restaurant => {
        html += `
            <div class="restaurant-card" onclick="fetchRestaurantDetails(${restaurant.id})">
...
async function fetchRestaurantDetails(restaurantId) {
    ...
    restaurantDetailContainer.innerHTML = '<div class="loading">Loading restaurant details...</div>';
    menuItemsContainer.innerHTML = '<div class="loading">Loading menu...</div>';
    
    try {
        // Fetch restaurant details
        const restaurantResponse = await fetch(`${API_ENDPOINTS.restaurants}/${restaurantId}`);
        ...
        menuItemsByCategory[category].forEach(item => {
            html += `
                <div class="menu-item">
                    ...
                    <button class="add-to-cart" onclick="addToCart(${item.id}, '${item.name}', ${item.price})">+</button>
                </div>
            `;
        });
...
function displayOrderConfirmation(order) {
    const confirmationDetailsContainer = document.getElementById('order-confirmation-details');
    ...
    orders.forEach(order => {
        html += `
            <div class="order-card" onclick="fetchOrderDetails(${order.id})">
                ...
                <div class="order-restaurant">Restaurant: ${getRestaurantName(order.restaurantId)}</div>
...
async function fetchOrderDetails(orderId) {
    ...
    orderDetailContainer.innerHTML = '<div class="loading">Loading order details...</div>';
...
function displayOrderDetails(order) {
    ...
    const html = `
        <div class="restaurant-detail">
            ...
            <div class="order-status status-${order.status}" style="margin: 10px 0;">${formatOrderStatus(order.status)}</div>
            
            <div style="margin-bottom: 20px;">
                <div><strong>Restaurant:</strong> ${getRestaurantName(order.restaurantId)}</div>
